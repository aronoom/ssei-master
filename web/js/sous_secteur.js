...
$(function(){
    ...
    $('#indicateurTable').dataTable({
        "language": {
            "lengthMenu": "Afficher _MENU_ sous secteur",
            "zeroRecords": "Aucun sous secteur trouvée - vérifier son numero ou son libellé",
            "info": "Page _PAGE_ sur _PAGES_",
            "infoEmpty": "Aucune sous secteur",
        ...
        "ajax":
        {
            "url": Routing.generate('sous_secteur_listeAjax'),
            "dataSrc": ""
        },
        "columns": [
            { "data":"id" },
            { "data": "libelleSousSecteur" },
            { "data": "" },
        ],
        "columnDefs": [ {
            "targets": -1,
    ...
    $("#ajouterIndicateur").click(function(){
        $.ajax({
            type: 'GET',
            url: Routing.generate('sous_secteur_ajouter'),
            timeout: 3000,
            success: function(data) {
                $('.modal-body').children().remove();
                $('.modal-header h4').text("Ajouter un sous secteur");
    ...
    $('#indicateurTable tbody').on( 'click', 'button', function () {
            if($(this).is('.btn-editIndicateur'))
            {
                ...
                $.ajax({
                    type: 'GET',
                    url: Routing.generate('sous_secteur_modifier',{ id : $id.text()}),
                    timeout: 30000,
                    success: function (response) {
                        ...
                        $('.modal-header h4').text("Modifier le sous secteur"+$id.text());
               ...
               <h4 class='modal-title' data-noInd='' >Modification</h4>\n\
                ...
                <div class='modal-body'>\n\
                    <div class='alert alert-danger text-center' stlye='border-radius:10px;'>\n\n\
                        <h5> Valider la suppression du sous secteur"+ $id.text() +"?</h5></div>\n\
                    </div>\n\
                ...
                $('#supModal .modal-header h4').text("Supprimer le sous secteur n° "+ $id.text()) ;
                $('#supModal').modal('show');

                $('#btn-valider').click(function(){
                    $.ajax({
                        type: 'POST',
                        url: Routing.generate('sous_secteur_supprimer'),
                        data : {
