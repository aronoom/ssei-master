...
$(function(){
    ...
    $('#indicateurTable').dataTable({
        "language": {
            "lengthMenu": "Afficher _MENU_ mode_de_calcul",
            "zeroRecords": "Aucun mode de calcul trouvée - vérifier son numero ou son libellé",
            "info": "Page _PAGE_ sur _PAGES_",
            "infoEmpty": "Aucun mode de calcul",
        ...
        "ajax":
        {
            "url": Routing.generate('mc_listeAjax'),
            "dataSrc": ""
        },
        "columns": [
            { "data":"id" },
            { "data": "libelleModeCalcul" },
            { "data": "" },
        ],
        "columnDefs": [ {
            "targets": -1,
    ...
    $("#ajouterIndicateur").click(function(){
        $.ajax({
            type: 'GET',
            url: Routing.generate('mc_ajouter'),
            timeout: 3000,
            success: function(data) {
                $('.modal-body').children().remove();
                $('.modal-header h4').text("Ajouter un mode de calcul");
    ...
    $('#indicateurTable tbody').on( 'click', 'button', function () {
            if($(this).is('.btn-editIndicateur'))
            {
                ...
                $.ajax({
                    type: 'GET',
                    url: Routing.generate('mc_modifier',{ id : $id.text()}),
                    timeout: 30000,
                    success: function (response) {
                        ...
                        $('.modal-header h4').text("Modifier le mode de calcul "+$id.text());
            ...
            else
            {
               ...
               <h4 class='modal-title' data-noInd='' >Modification</h4>\n\
                ...
                $('#supModal .modal-header h4').text("Supprimer le mode de calcul n° "+ $id.text()) ;
                $('#supModal').modal('show');

                $('#btn-valider').click(function(){
                    $.ajax({
                        type: 'POST',
                        url: Routing.generate('mc_supprimer'),
                        data : {
