...
(function (global, factory) {
  ...
  Viewer.prototype = {
    ...
    renderImage: function (callback) {
      ...
      var name = _this.image.alt;
      $('#anomalieBtn').attr('data-page',name);
    ...
    next: function () {
      ...
      $.ajax({
        ...
        {
            $.unblockUI();
            if($(".caption:eq("+index+") p span").length == 0)     
              $(".caption:eq("+index+") p strong").append("<span class='label label-success'>Contrôlée</span>");            
            if(response.page == response.total)
            {
                _this.hide();  
                alert("Fin d'image.Marquer le contrôle comme fini si toutes les images ont été contrôlées.\nLe boutton se trouve en haut de la page.");
                return;                
            }
        },
        error: function(response)
        {
            ...
            $('#globModal .modal-body').append(response.responseText);
            $('#globModal .modal-header h4').text("Internal error");            
            $('#globModal').modal('show');
            
            
        }
      });
     if(_this.length == ( _this.index +1))
     {
      if($(".caption:eq("+index+") p span").length == 0)     
        $(".caption:eq("+index+") p strong").append("<span class='label label-success'>Contrôlée</span>");         
        _this.hide();  
