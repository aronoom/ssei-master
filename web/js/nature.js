...
$(function(){
    ...
    $('#indicateurTable').dataTable({
        "language": {
            "lengthMenu": "Afficher _MENU_ nature",
            "zeroRecords": "Aucune nature trouvée - vérifier son numero ou son libellé",
            "info": "Page _PAGE_ sur _PAGES_",
            "infoEmpty": "Aucune nature",
        ...
        "ajax":
        {
            "url": Routing.generate('nature_listeAjax'),
            "dataSrc": ""
        },
        "columns": [
            { "data":"id" },
    ...
    $("#ajouterIndicateur").click(function(){
        $.ajax({
            type: 'GET',
            url: Routing.generate('nature_ajouter'),
            timeout: 3000,
            success: function(data) {
                $('.modal-body').children().remove();
                $('.modal-header h4').text("Ajouter une nature");
    ...
    $('#indicateurTable tbody').on( 'click', 'button', function () {
            if($(this).is('.btn-editIndicateur'))
            {
                ...
                $.ajax({
                    type: 'GET',
                    url: Routing.generate('nature_modifier',{ id : $id.text()}),
                    timeout: 30000,
                    success: function (response) {
                        ...
                        $('.modal-header h4').text("Modifier la nature"+$id.text());
               ...
               <h4 class='modal-title' data-noInd='' >Modification</h4>\n\
                ...
                <div class='modal-body'>\n\
                    <div class='alert alert-danger text-center' stlye='border-radius:10px;'>\n\n\
                        <h5> Valider la suppression de la nature"+ $id.text() +"?</h5></div>\n\
                    </div>\n\
                ...
                $('#supModal .modal-header h4').text("Supprimer la nature n° "+ $id.text()) ;
                $('#supModal').modal('show');

                $('#btn-valider').click(function(){
                    $.ajax({
                        type: 'POST',
                        url: Routing.generate('nature_supprimer'),
                        data : {
