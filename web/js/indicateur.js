...
$(function(){
    ...
    $('#indicateurTable').dataTable({
        "language": {
            "lengthMenu": "Afficher _MENU_ indicateurs",
            "zeroRecords": "Aucun indicateur trouvé - vérifier son numero ou son libellé",
            "info": "Page _PAGE_ sur _PAGES_",
            "infoEmpty": "Aucun indicateur",
        ...
        "ajax":
        {
            "url": Routing.generate('indicateur_listeAjax'),
            "dataSrc": ""
        },
        "columns": [
            { "data":"id" },
            { "data":"libelleIndicateur" },
            { "data": "nature.libelleNature" },
            { "data": "unite.libelleUnite" },
            { "data": "activite.libelleActivite" },
            { "data": "periodicite.libellePeriodicite" },
            { "data": "modeCalcul.libelleModeCalcul" },
            { "data": "sousSecteur.libelleSousSecteur" },
            { "data": "subdivision.libelleSubdivision" },
            { "data": "" },
        ],
        "columnDefs": [ {
            "targets": -1,
    ...
    $("#ajouterIndicateur").click(function(){
        $.ajax({
            type: 'GET',
            url: Routing.generate('indicateur_ajouter'),
            timeout: 3000,
            success: function(data) {
                $('.modal-body').children().remove();
                $('.modal-header h4').text("Ajouter un indicateur");
    ...
    $('#indicateurTable tbody').on( 'click', 'button', function () {
            if($(this).is('.btn-editIndicateur'))
            {
                ...
                $.ajax({
                    type: 'GET',
                    url: Routing.generate('indicateur_modifier',{ id : $id.text()}),
                    timeout: 30000,
                    success: function (response) {
                        ...
                        $('.modal-header h4').text("Modifier l'indicateur "+$id.text());
               ...
               <h4 class='modal-title' data-noInd='' >Modification</h4>\n\
                ...
                <div class='modal-body'>\n\
                    <div class='alert alert-danger text-center' stlye='border-radius:10px;'>\n\n\
                        <h5> Valider la suppression de l'indicateur"+ $id.text() +"?</h5></div>\n\
                    </div>\n\
                ...
                $('#supModal .modal-header h4').text("Supprimer l'indicateur n° "+ $id.text()) ;
                $('#supModal').modal('show');

                $('#btn-valider').click(function(){
                    $.ajax({
                        type: 'POST',
                        url: Routing.generate('indicateur_supprimer'),
                        data : {
