...
$(function(){
    ...
    $('#indicateurTable').dataTable({
        "language": {
            ...
            "zeroRecords": "Aucune activité trouvée - vérifier son numero ou son libellé",
        ...
        "ajax":
        {
            "url": Routing.generate('activite_listeAjax'),
            "dataSrc": ""
        },
        "columns": [
            { "data":"id" },
            { "data": "libelleActivite" },
            { "data": "" },
        ],
        "columnDefs": [ {
            "targets": -1,
    ...
    $("#ajouterIndicateur").click(function(){
        $.ajax({
            ...
            success: function(data) {
                $('.modal-body').children().remove();
                $('.modal-header h4').text("Ajouter une activité");
    ...
    $('#indicateurTable tbody').on( 'click', 'button', function () {
            if($(this).is('.btn-editIndicateur'))
            {
                ...
                $.ajax({
                    type: 'GET',
                    url: Routing.generate('activite_modifier',{ id : $id.text()}),
                    timeout: 30000,
                    success: function (response) {
                        ...
                        $('.modal-header h4').text("Modifier l'activite "+$id.text());
            ...
            else
            {
               ...
               <h4 class='modal-title' data-noInd='' >Modification</h4>\n\
                ...
                $('#supModal .modal-header h4').text("Supprimer l'activite n° "+ $id.text()) ;
                $('#supModal').modal('show');

                $('#btn-valider').click(function(){
                    $.ajax({
                        type: 'POST',
                        url: Routing.generate('activite_supprimer'),
                        data : {
